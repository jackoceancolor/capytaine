# -*- coding: utf-8 -*-
"""
Created on Thu Dec  3 15:27:01 2020
...
rm3_w = np.linspace(0.02, 5.2, 2)                    # wave frequencies. 260 for full run
rm3_headings = np.linspace(0,0,1)                    # wave headings
rm3_depth = np.infty                                 # water depth

rm3_ncFile = os.getcwd() + os.path.sep + 'gbm_test_revdofs.nc'   # path for output .nc file
rm3_gbm_dir = os.getcwd()                            # path for gbm dof directory
# ----------------------------------------------------------------------------#
...
# Run Capytaine
cd, p = cc.call_capy_gbm(meshFName = rm3_file,
              wCapy     = rm3_w,
              CoG       = rm3_cg,
              ...
              depth     = rm3_depth,
              density   = 1000.0,
              additional_dofs_dir = rm3_gbm_dir)
