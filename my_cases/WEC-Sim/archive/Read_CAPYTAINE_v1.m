function hydro = Read_capytaine_v1(hydro,filename)
...
% 
% todo
% - Body-to-body interaction is not supported. 
%       The off diagonal 6x6 matrices for added mass, damping, stiffness are 0
...
%   implemented?)
% - GBM should be supported. Has not been tested here
% 
...
% todo - Capytaine doesn't currently have hydrostatics data, use mesh
% function to calculate?
hydro(F).cg = zeros(3,hydro(F).Nb); % center of gravity
hydro(F).cb = zeros(3,hydro(F).Nb) - [0; 0; 1e-2]; % center of buoyancy
hydro(F).Vo = zeros(1, hydro(F).Nb); % displaced volume
...
hydro(F).dof = dof_i; % 6 normally, >6 with GBM
waitbar(2/8);
...
% todo - Capytaine doesn't currently include hydrostatics output
...
if contains(lower(cpt_vars), 'hydrostatic_stiffness')
...
% ('old_dofs') are not in the correct order: [surge, sway, heave, roll, pitch, yaw]
