function hydro = Read_CAPYTAINE_b2b_v1(hydro,filename)
...
% this version adds b2b capability (6*Nb dofs) but without GBM and using
% the same format as _v4. _b2b_v2 instead usses the sum() to combine the
% body dimension
...
tmp = [];
if length(hydro(F).body)>1
    for i=1:length(hydro(F).body)
        tmp = [tmp strcat(hydro(F).body{i},'__',std_dofs)];
    end
    std_dofs = tmp';
end
clear tmp
...
reset_idofs = true;
if reset_idofs
    tmp = reset_dofs(tmp,idofs,1,hydro(F).body);
end
if reset_rdofs
    tmp = reset_dofs(tmp,rdofs,2,hydro(F).body);
end
