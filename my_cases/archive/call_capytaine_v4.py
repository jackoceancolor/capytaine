...
def call_capy_b2b(meshFName, wCapy, CoG=([0,0,0],), headings=[0.0], saveNc=False,
    ...
    # add hydrostatics data to combined body
    # vo = np.zeros([1*len(bodies)])
    # cg = np.zeros([3*len(bodies)])
    # cb = np.zeros([3*len(bodies)])
    # khs = np.zeros([6*len(bodies)])
    # for i in np.arange(0,len(bodies)):
    #     vo[i] = bodies[i].displaced_volume
    #     cg[i*3:(i+1)*3] = bodies[i].center_of_mass
    #     cb[i*3:(i+1)*3] = bodies[i].center_of_buoyancy
    #     khs[i*6:(i+1)*6] = bodies[i].hydrostatic_stiffness
    # combo.hydrostatic_stiffness = xr.DataArray(khs)
    # combo.displaced_volume = xr.DataArray(vo)
    # combo.center_of_mass = xr.DataArray(cg)
    # combo.center_of_buoyancy = xr.DataArray(cb)
