function hydro = Read_CAPYTAINE_b2b_v3(hydro,filename)
...
[r_sorted_dofs,rinds,reset_rdofs] = sorted_dof_list(rdofs, hydro(F).body);

% sort radiating dof into standard list if necessary
idofs = lower(string(ncread(filename,'influenced_dof')'));
idofs = erase(idofs, char(0));
[i_sorted_dofs,iinds,reset_idofs] = sorted_dof_list(idofs, hydro(F).body);
